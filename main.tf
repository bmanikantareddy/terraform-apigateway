resource "oci_apigateway_gateway" "this" {
  ...
  dynamic "ca_bundles" {
    for_each = lookup(var.gateway[count.index], "ca_bundles", [])
    content {
        type                     = lookup(ca_bundles.value, "type", null)
        ca_bundle_id             = lookup(ca_bundles.value, "ca_bundle_id", null)
        certificate_authority_id = lookup(ca_bundles.value, "certificate_authority_id", null)
    }
  }
        ...
        content {
          dynamic "authentication" {
            ...
            #   audiences                   = lookup(authentication.value, "audiences")
              audiences                   = oci_apigateway_gateway.this.hostname
